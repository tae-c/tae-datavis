...
I've learned through class critique that I could leverage "If" statements in the code overrides within Flourish if I wanted to achieve displaying 4 colors for each surface category. However, after a while of wrestling with Flourish, I was not able to figure out how to achieve the desired look. Althought not exactly how I imagined, the current version as displayed still achieves a portion of my initial intent. I would have wanted the viewer to take away from looking at one visual that grass courts are typically faster and clay courts are slower. However, the viewer will need to click between the 4 tabs, but with the information that shows for each tab, they are still able to extract that information. Also by clicking between the four tabs, the viewer can see that there are not many grass tournaments in the pro tour, but instead they will realize that majority of the pro tours are played on clay and hard court surfaces.
